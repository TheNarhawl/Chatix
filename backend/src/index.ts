...
app.post('/chat/create-dm', async (req: Request, res: Response) => {
    ...
    if (!senderUsername || !getterUsername) {
        return res.status(400).json({ error: "Требуется senderUsername и getterUsername" });
    }
    ...
    try {
        ...
        const existingChat = await chatRepository
            ...
            .innerJoin("chat.users", "user1", "user1.username = :senderUsername", { senderUsername })
            .innerJoin("chat.users", "user2", "user2.username = :getterUsername", { getterUsername })
